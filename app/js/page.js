...
function pageTurn(val) {
  ...
  // Limits Val to range
  if (index + val >= dirArray.length -1) { // For last page
    index = dirArray.length -1;
    val = 0;
    polarity = 0;
    singlePage(fileDir, dirArray, index);
...
function singlePage(fileDir, dirArray, index) { // For Single page viewing and styling
  var viewOne = document.getElementById('viewImgOne');
  var viewTwo = document.getElementById('viewImgTwo');
